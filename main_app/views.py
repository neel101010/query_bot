...
def similarmatch(request , course_id, video_id):
    # Based on course_id find all Q&A
    # Then based on question do match
    print(request.POST.get('question'))
    question = request.POST.get('question')
    allQuestions = []
    allAnswers = []
    AllQuestions = Qna.objects.all()
    for questionpair in AllQuestions:
        allQuestions.append(questionpair.question)
        allAnswers.append(questionpair.answer)

    data = predict_similar_questions(question , allQuestions , allAnswers)
    print(f"Got response from similar questions ===> {data}")
    # data = {
    #     "ismatched": False,
    #     "matchedanswer": "Job hunting experience involves networking to get in touch with the right people in various companies, applying to lots of jobs through various channels, preparing for interviews – while interviews are uncertain it is necessary",
    #     "matchedquestion": "What does the job hunting experience look like ?",
    #     "percentmatched": 0.8691164777191123,
...
# Last step of generating answer
@csrf_exempt
...
@csrf_exempt
def question_post(request , course_id , video_id):
   if request.method == 'POST':
       question = request.POST.get('question')
       new_qna = Qna(question=question, course_id=course_id, video_id=video_id)
       new_qna.save()
       print(new_qna)
       return JsonResponse({
            'isposted' : True,
